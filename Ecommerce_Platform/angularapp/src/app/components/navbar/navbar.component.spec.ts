...
describe('NavbarComponent', () => {
  ...
  fit('Frontend_NavbarComponent_should_have_initial_state_with_cartCount=0_and_not_logged_in', () => {
  ...
  fit('Frontend_NavbarComponent_should_logout_user_and_reset_states', () => {
    spyOn(window, 'confirm').and.returnValue(true); // simulate Yes
  ...
  fit('Frontend_NavbarComponent_should_not_logout_if_user_cancels', () => {
    spyOn(window, 'confirm').and.returnValue(false); // simulate No
    localStorage.setItem('currentUser', JSON.stringify({ role: 'user' }));
    component.isLoggedIn = true;
    component.logout();
